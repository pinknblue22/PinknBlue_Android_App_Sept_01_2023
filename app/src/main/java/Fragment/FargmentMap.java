...
public class FargmentMap extends Fragment implements OnMapReadyCallback , GoogleMap.OnMarkerClickListener {
    ...
    private static final String TAG = FargmentMap.class.getSimpleName();
...
//        }
        ...
        //  getLocationFromAddress(getActivity(),"mohali punjab");
        return view;
    }

    private void HitApi() {
        Log.e(TAG, "HitApi: " + "schedule_id :"+ schedule_id);
...
*/
                   for (int i=0;i<data.getHotspots().size();i++){
                       ...
                       googleMap.setOnMarkerClickListener(FargmentMap.this);
                   }


                    for (int idx = 0; idx < data.getStopdatalist().size(); idx++) {
//                        String address = data.getStopdatalist().get(idx).getStop_address();
                        String strAddress = data.getStopdatalist().get(idx).getOrig_address();
                       /* Log.d("getLatitude-->",data.getStopdatalist().get(idx).getLatitude());
                        Log.d("getLongitude-->",data.getStopdatalist().get(idx).getLongitude());*/
                       /* double latitude = Double.valueOf(data.getStopdatalist().get(idx).getLatitude());
                        double longitude = Double.valueOf(data.getStopdatalist().get(idx).getLongitude());*/
//                        getLocationFromAddress(getActivity(), address,idx,data.getStopdatalist(),9.5114963, 76.3543222);
                        /*if (!data.getStopdatalist().get(idx).getLatitude().isEmpty() || !data.getStopdatalist().get(idx).getLongitude().isEmpty()) {
                            double latitude = Double.valueOf(data.getStopdatalist().get(idx).getLatitude());
                            double longitude = Double.valueOf(data.getStopdatalist().get(idx).getLongitude());
                            getLocationFromAddress(getActivity(), address, idx, data.getStopdatalist(), latitude, longitude);
                        }*/

//                        if (data.getStopdatalist().get(idx).getLatitude() != "" || !data.getStopdatalist().get(idx).getLongitude().isEmpty()) {
...
//        Toast.makeText(getActivity(), s, Toast.LENGTH_SHORT).show();
    ...
    /*public LatLng getLocationFromAddress(Context context, String strAddress, int idx, List<ServerResponseScheduledetail.Stop_data> stopdatalist, double latitude, double longitude) {
...
//            p1 = new LatLng(latitude, longitude);
            ...
            if(idx==0){
                ...
                googleMap.addMarker(new MarkerOptions().icon(BitmapDescriptorFactory.fromResource(R.mipmap.ic_source_icon)).position(p1));
                googleMap.moveCamera(CameraUpdateFactory.newLatLngZoom(p1, 13));
            }else {

                if (stopdatalist.size() - 1 == idx) {
                    googleMap.addMarker(new MarkerOptions().icon(BitmapDescriptorFactory.fromResource(R.mipmap.ic_destination_icon)).position(p1));
                } else {
                    googleMap.addMarker(new MarkerOptions().icon(BitmapDescriptorFactory.fromResource(R.mipmap.bus_stop)).position(p1));
                }
            }
    ...
    private void hitRoute() {
        ...
        FargmentMap.GetRouteTask getRoute = new FargmentMap.GetRouteTask();
        getRoute.execute();
    }
...
//                PolylineOptions rectLine = new PolylineOptions().width(10).color(Color.parseColor("#457DD7"));
                PolylineOptions rectLine = new PolylineOptions().width(10).color(Color.parseColor("#0FB0EC"));
